...
function makeStatTrakDisplay(id){
  ...
  const mesh = new THREE.Mesh(new THREE.PlaneGeometry(0.19, 0.047), material);
  // Mounted on the camera-facing side of the receiver. Utility/knife use a compact floating tag.
  mesh.position.set(id === 'knife' ? 0.20 : 0.285, id === 'knife' ? -0.18 : -0.135, id === 'knife' ? -0.31 : -0.43);
  mesh.rotation.y = -0.12;
  return { id, canvas, texture, mesh };
}
function updateStatTrakDisplay(visual){
  ...
  ctx.fillStyle = 'rgba(18,20,18,.94)'; ctx.fillRect(0, 0, 256, 64);
  ctx.strokeStyle = '#4d5148'; ctx.lineWidth = 4; ctx.strokeRect(2, 2, 252, 60);
  ctx.fillStyle = '#ff8a18'; ctx.font = 'bold 20px monospace'; ctx.fillText('STATTRAK', 10, 22);
  ctx.font = 'bold 29px monospace'; ctx.textAlign = 'right';
  ctx.fillText(String(playerProfile.weaponKills?.[st.id] || 0).padStart(6, '0'), 246, 53);
  ctx.textAlign = 'left'; st.texture.needsUpdate = true;
}
...
function buildWeaponVisual(id){
  ...
  if (statTrak) group.add(statTrak.mesh);
  const visual = {
